...
customer_order_history as (
  ...
  select 
     ...
     customers.givenname,
      ...
      sum(case when orders.order_status NOT IN ('returned','return_pending') then ROUND(payments.amount/100.0,2) else 0 end) as total_lifetime_value,
      sum(case when orders.order_status NOT IN ('returned','return_pending') then ROUND(payments.amount/100.0,2) else 0 end)
